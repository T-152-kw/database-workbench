...
- **通知中心** - 新增通知中心面板，自动记录历史操作通知，支持未读提醒和一键清空
- **SQL 查询增强** - 单条查询快速路径优化，批量脚本执行优化，智能事务处理，虚拟滚动提升海量结果流畅度
- **连接管理优化** - 连接导入导出支持冲突检测和覆盖选择，收藏夹支持自动打开连接
- **ER 图导出升级** - PDF 导出内置中文字体支持，矢量图形渲染，智能缩放确保清晰度
- **主题一致性** - 修复对话框等特殊场景的背景色匹配问题
